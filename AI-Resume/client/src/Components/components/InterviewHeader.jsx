...
const InterviewHeader = ({ interviewState, faceDetection = {} }) => {
  ...
  const getFaceStatusColor = () => {
    if (!safeFaceDetection.faceDetectionStatus) return "bg-gray-500";
    
    switch(safeFaceDetection.faceDetectionStatus) {
  ...
  const getFaceStatusText = () => {
    if (!safeFaceDetection.faceDetectionStatus) return "🔍 Face Detection";
    
    switch(safeFaceDetection.faceDetectionStatus) {
      case "active": return safeFaceDetection.isFaceDetected ? "✅ Face Detected" : "❌ No Face";
  ...
  return (
    <>
      ...
      {interviewState.faceDetectionEnabled && safeFaceDetection.faceDetectionStatus === "active" && (
        <div className="fixed top-4 left-32 z-40">
          <div className={`px-3 py-2 rounded-lg shadow-lg ${getFaceStatusColor()} text-white`}>
            <div className="flex items-center gap-2">
              ...
              {safeFaceDetection.stats?.confidenceScore > 0 && (
                <span className="text-xs bg-black bg-opacity-20 px-1 rounded">
                  {safeFaceDetection.stats.confidenceScore.toFixed(1)}%
                </span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">
                ...
                {safeFaceDetection.faceDetectionStatus === "active" ? " • 🔍 Face Detection Active" : ""}
              </p>
            </div>
          </div>
